...
计时模式陷阱（重要）：轮询用 usleep(1000)，**单命令计时地板 ~1ms**。
低速档（5000 时钟 × 439ns ≈ 2.2ms）没问题；快时钟（R30 实验 iters=0，
290ns × 5000 = 1.45ms）会被地板污染读成 440ns。规则：**总时长必须
≥3× 地板**（加大 clocks），或用分支上加了忙轮询 `exec_cmd_busy` 的
版本。
